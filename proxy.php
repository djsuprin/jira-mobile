...
if ( !$url ) {
	...
	/*
		djsuprin: I couldn't manage to resend Authorization header from initial request to the destination server using CURLOPT_HTTPHEADER
		so I parse its value, decode credentials and set it to CURLOPT_USERPWD. It works but I feel there must be a better solution.
	*/
	...
	foreach (getallheaders() as $name => $value) {
		...
		if ($name === 'Authorization') {
			$hash = base64_decode( substr($value, strpos($value, 'Basic ') + 6) );
			break;
		} else continue;
	}
	
  
	//curl_setopt( $ch, CURLOPT_HTTPHEADER, $hdrs[] = $headers[4] );
